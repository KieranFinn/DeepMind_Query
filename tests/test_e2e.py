...
async def test_create_branch(authenticated_page: Page):
    ...
    try:
        ...
        # Look for a new button with "分支" text (exactly one branch expected after double-click)
        branch_nodes = await page.query_selector_all("button:has-text('分支')")
        assert len(branch_nodes) == 1, f"Expected exactly one branch node after double-click, found {len(branch_nodes)}"
...
async def test_double_click_node(authenticated_page: Page):
    ...
    try:
        ...
        # Verify exactly one branch was created (double-click creates one child node)
        assert len(branch_buttons) == 1, f"Expected exactly one branch node after double-click, found {len(branch_buttons)}"
